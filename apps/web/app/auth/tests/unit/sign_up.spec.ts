...
function createAuthSpy() {
  ...
  return { auth: auth as unknown as Parameters<SignUp['handle']>[0]['auth'], calls }
}

test.group('SignUp', (group) => {
  group.each.setup(() => testUtils.db().wrapInGlobalTransaction())

  test('cria o usuario com senha hasheada e faz login', async ({ assert }) => {
    ...
    assert.equal(persisted.fullName, 'Filipe Teste')
