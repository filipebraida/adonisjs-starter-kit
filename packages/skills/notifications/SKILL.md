...
For resource-scoped realtime (job progress, live status) that isn't a per-user Facteur notification — hardcoded channel pattern, `transmit.authorize`, broadcast from a listener → [references/custom-sse-channel.md](references/custom-sse-channel.md).
